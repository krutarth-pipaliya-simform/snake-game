...
- Folder structure:
  ...
  src/
    ...
    realtime/       — Supabase channel setup and broadcast/receive logic
...
- Supabase channel calls (`subscribe`, `send`) should have basic error/failure
  handling (e.g. log and show a "connection lost" indicator) — this is a casual
  multiplayer game, not a production service, so a full retry/backoff system is
  not required, but silent failures are not acceptable either.

## What NOT to do

- Do not add libraries beyond what's specified (Supabase, RTK, Tailwind, plus
  standard React tooling) without checking with the user first — extra
  dependencies slow down a one-day build.
- Do not add server-side code/backend beyond Supabase itself — there is no custom
  backend in this project (see the project skill for why).
